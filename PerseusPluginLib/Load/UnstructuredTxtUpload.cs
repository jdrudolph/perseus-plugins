...
namespace PerseusPluginLib.Load{
    public class UnstructuredTxtUpload : IMatrixUpload{
        ...
        public Bitmap DisplayImage{
            get { return Resources.raw; }
        }

        public string Name{
            get { return "Raw upload"; }
        }
        ...
        public Parameters GetParameters(ref string errString){
            return
                new Parameters(new Parameter[]{
                    new FileParam("File"){
                        Filter =
                            "Text (Tab delimited) (*.txt)|*.txt|CSV (Comma delimited) (*.csv)|*.csv|All files (*.*)|*.*",
                    ...
                    new BoolWithSubParams("Split into columns", false){
                        SubParamsTrue =
                            new Parameters(new SingleChoiceParam("Separator"){Values = new[]{"Tab", "Comma"}})
                    }
                });
        }
        ...
        private static void LoadNoSplit(IMatrixData mdata, string filename){
            ...
            reader.Close();
            mdata.SetData("", "", new List<string>(), new List<string>(), new float[lines.Count,0],
                          new bool[lines.Count,0], new float[lines.Count,0], "", true,
                          new List<string>(new[]{"All data"}),
        ...
        private static void LoadSplit(IMatrixData mdata, string filename, bool csv){
            ...
            mdata.SetData("", "", new List<string>(), new List<string>(), new float[nrows,0], new bool[nrows,0],
                          new float[nrows,0], "", true, new List<string>(colNames), new List<string>(colNames),
                          new List<string[]>(cols), new List<string>(), new List<string>(), new List<string[][]>(),
                          new List<string>(), new List<string>(), new List<double[]>(), new List<string>(),
