    ...
    public class MatchingRowsByName : IMatrixMultiProcessing{
        ...
        public IMatrixData ProcessData(IMatrixData[] inputData, Parameters parameters, ref IMatrixData[] supplTables,
            ...
            Func<double[], double> avExpression =
                GetAveraging(parameters.GetSingleChoiceParam("Combine expression values").Value);
            Func<double[], double> avNumerical =
                GetAveraging(parameters.GetSingleChoiceParam("Combine numerical values").Value);
            ...
            }
                                           {
                                               ...
                                               for (int i = 0; i < exCols.Length; i++){
                                                   ...
                                                   for (int j = 0; j < mdata1.RowCount; j++){
                                                       ...
                                                       newExColumns[j, i] = values.Count == 0
                                                                                ? float.NaN
                                                                                : (float) avExpression(values.ToArray());
                                                       newQuality[j, i] = qual.Count == 0
                                                                              ? float.NaN
                                                                              : (float) avExpression(qual.ToArray());
                                                       newIsImputed[j, i] = imp.Count != 0 && AvImp(imp.ToArray());
                                                   }
                                               }
                                               MakeNewNames(newExColNames, result.ExpressionColumnNames);
                                               AddExpressionColumns(result, newExColNames, newExColumns, newQuality,
                                                                    newIsImputed);
                                           }
                                           {
                                               ...
                                               if (avNumerical != null){
                                                   ...
                                                   for (int i = 0; i < numCols.Length; i++){
                                                       ...
                                                       for (int j = 0; j < mdata1.RowCount; j++){
                                                           ...
                                                           newNumericalColumns[i][j] = values.Count == 0
                                                                                           ? double.NaN
                                                                                           : avNumerical(
                                                                                               values.ToArray());
                                                       }
                                                   ...
                                                   for (int i = 0; i < numCols.Length; i++){
                                                       result.AddNumericColumn(newNumColNames[i], "",
                                                                               newNumericalColumns[i]);
                                                   }
                                               } else{
                                                   double[][][] newMultiNumericalColumns =
                                                       new double[numCols.Length][][];
                                                   ...
                                                   for (int i = 0; i < numCols.Length; i++){
                                                       result.AddMultiNumericColumn(newMultiNumColNames[i], "",
                                                                                    newMultiNumericalColumns[i]);
                                                   }
                                               }
                                           }
                                           {
                                               int[] catCols =
                                                   parameters.GetMultiChoiceParam("Categorical columns").Value;
                                                   ...
                                                   for (int j = 0; j < mdata1.RowCount; j++){
                                                       ...
                                                       newCatColumns[i][j] = values.Count == 0
                                                                                 ...
                                                                                 : ArrayUtils.UniqueValues(
                                                                                     ArrayUtils.Concat(values.ToArray()));
                                                   }
                                               }
                                               for (int i = 0; i < catCols.Length; i++){
                                           ...
                                           }
                                           {
                                               ...
                                               for (int i = 0; i < stringCols.Length; i++){
                                                   ...
                                                   for (int j = 0; j < mdata1.RowCount; j++){
                                                       ...
                                                       newStringColumns[i][j] = values.Count == 0
                                                                                    ? ""
                                                                                    : StringUtils.Concat(";",
                                                                                                         values.ToArray());
                                                   }
        ...
        public static void AddExpressionColumns(IMatrixData data, string[] names, float[,] vals, float[,] qual,
                                                bool[,] imp){
