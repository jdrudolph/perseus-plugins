...
namespace PluginProteomicRuler{
	public class AnnotateSequences : IMatrixProcessing{
		...
		public string Description{
			get{
				return "Annotate proteins using information extracted from a (uniprot) fasta file.\n" +
		...
		public void ProcessData(IMatrixData mdata, Parameters param, ref IMatrixData[] supplTables,
			...
			if (ArrayUtils.Contains(selection, 2))
				// Verbose protein name, i.e. all protein names annotated in all fasta headers, including the 'Isoform x of...' prefixes and '(Fragment)' suffixes
			{
				string[] annotationColumn = new string[mdata.RowCount];
				for (int row = 0; row < mdata.RowCount; row++){
					List<string> rowAnnotations = new List<string>();
			...
			if (param.GetParamWithSubParams<int>("Count sequence features").GetSubParameters().GetParam<string>("Regex").Value !=
				...
				mdata.AddNumericColumn(
					(normalizeBySequenceLength ? "Normalized feature count (" : "Feature count (") + regex.ToString() + ")", "",
			...
			return
				new Parameters(new Parameter[]{
					...
					new FileParam("Fasta file"){
						...
						Help =
							"Select the fasta file used for the database search of this dataset. The software will assume uniprot-formatted headers for extracting accession IDs and metadata. As fallback position, everything after the > will be taken as ID, but no header metadata can be extracted from non-uniprot headers."
					},
							...
							new List<Parameters>(){
								new Parameters(new Parameter[]{
									new MultiChoiceParam("Annotations"){
										Values =
											new string[]{"Entry name", "Gene name", "Protein name (verbose)", "Protein name (consensus)", "Species"},
										Value = new int[]{1, 3},
									}
								}),
								new Parameters(new Parameter[]{
					...
					new SingleChoiceWithSubParams("Calculate theoretical peptides"){
						...
						SubParams =
							new List<Parameters>(){
								new Parameters(new Parameter[]{
									new MultiChoiceParam("Proteases"){Values = Constants.DefaultProteasesNames(), Value = new int[]{0}},
					...
					new SingleChoiceWithSubParams("Count sequence features"){
						...
						TotalWidth = 500,
						Help =
							"Count the number of matches to a given regular expression in the amino acid sequence (optionally normalized by sequence length).\n\nExamples:\n[KR] tryptic cleavage sites\nN[^P][ST][^P] N-glysosylation motifs",
						Values = new string[]{"median of all IDs", "for the leading ID"},
						Value = 0,
						SubParams =
							new List<Parameters>(){
