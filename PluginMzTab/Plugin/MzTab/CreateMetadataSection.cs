...
namespace PluginMzTab.Plugin.MzTab{
	public class CreateMetadataSection : MzTabProcessing{
		...
		public override IMatrixData ProcessData(IMatrixData[] inputData, Parameters param, ref IMatrixData[] supplTables,
			...
			data.Name = Matrix.MetadataSection;
			data.ColumnNames = new List<string>();
			data.Values = new float[nrows,ncols];
			data.SetAnnotationColumns(columnames, columns, new List<string>(), new List<string[][]>(), new List<string>(),
				new List<double[]>(), new List<string>(), new List<double[][]>());
			data.ClearAnnotationRows();
			return data;
		}
		...
		private string GetParameterString(IMatrixData parameters, string name, string separator){
			...
			return separator == null ? value : StringUtils.Concat(separator, new[]{parameters.StringColumns[index][row], value});
		}
		...
		public override Parameters GetParameters(IMatrixData[] inputData, ref string errString){
			...
			AddSampleProcessingParameters(list, help, new List<SplitList<Lib.Model.Param>>(mtd.SampleProcessingMap.Values));
		...
		private void AddMzParameters(IList<Parameter> list, IList<ParameterDescription> help, Metadata mtd){
			...
			StringParam version = new StringParam(name, Constants.versions.Last()){Help = desc};
			list.Add(version);
					...
					new[]{
						new Parameters(new Parameter[]{
							new SampleProcessingParam(
								array != null && array.Length >= 1 ? ArrayUtils.SubArray(array, 1) : new SplitList<Lib.Model.Param>[1], cv)
						}),
						new Parameters(new Parameter[]{
							new SampleProcessingParam(
								array != null && array.Length >= 2 ? ArrayUtils.SubArray(array, 2) : new SplitList<Lib.Model.Param>[2], cv)
						}),
						new Parameters(new Parameter[]{
							new SampleProcessingParam(
								array != null && array.Length >= 3 ? ArrayUtils.SubArray(array, 3) : new SplitList<Lib.Model.Param>[3], cv)
						})
					}
			};
		...
		private void AddSoftware(IList<Parameter> list, List<Software> software, Dictionary<string, string> parameters,
			...
			group.SubParams[0] =
				new Parameters(new Parameter[]{
					new SoftwareParam(array != null && array.Length >= 1 ? ArrayUtils.SubArray(array, 1) : new Software[1], cv,
						parameters)
				});
			group.SubParams[1] =
				new Parameters(new Parameter[]{
					new SoftwareParam(array != null && array.Length >= 2 ? ArrayUtils.SubArray(array, 2) : new Software[2], cv,
						parameters)
				});
			group.SubParams[2] =
				new Parameters(new Parameter[]{
					new SoftwareParam(array != null && array.Length >= 3 ? ArrayUtils.SubArray(array, 3) : new Software[3], cv,
						parameters)
				});
			if (array != null && group.SubParams.Count >= array.Length){
				group.Value = array.Length - 1;
		...
		private void AddPublicationParameters(IList<Parameter> list, IList<Publication> publications,
			...
			for (int i = 0; i < count; i++){
				...
				group.SubParams[i] =
					new Parameters(
						new PublicationParam(array != null && array.Length >= n ? ArrayUtils.SubArray(array, n) : new Publication[n], cv));
			}
			group.Value = 0;
			if (array != null && group.SubParams.Count >= array.Length){
				group.Value = array.Length - 1;
		...
		private void AddContactParameters(IList<Parameter> list, IList<Contact> contacts, IList<ParameterDescription> help){
			...
			for (int i = 0; i < count; i++){
				...
				group.Values[i] = n.ToString(CultureInfo.InvariantCulture);
				group.SubParams[i] =
					new Parameters(new ContactParam(
						array != null && array.Length >= n ? ArrayUtils.SubArray(array, n) : new Contact[n], cv));
			}
			group.Value = 0;
			if (array != null && group.SubParams.Count >= array.Length){
				group.Value = array.Length - 1;
		...
		private void AddModificationParameters(IList<Parameter> list, IList<Mod> modifications, string name,
			...
			for (int i = 0; i < count; i++){
				...
				group.SubParams[i] =
					new Parameters(
						new ModificationParam(array != null && array.Length >= n ? ArrayUtils.SubArray(array, n) : new Mod[n], cv));
			}
			group.Value = 0;
			if (array != null && group.SubParams.Count >= array.Length){
				group.Value = array.Length - 1;
		...
		private void AddQuantificationParameters(IList<Parameter> list, Lib.Model.Param quantificationMethod,
			Lib.Model.Param protUnit, Lib.Model.Param pepUnit, IList<ParameterDescription> help){
			...
			SingleChoiceParam method = new SingleChoiceParam(name, index){Help = desc, Values = values};
			list.Add(method);
			...
			desc = null;
			name = CheckIfDescriptionExists(help, name, ref desc, null);
			index = MzTabMatrixUtils.GetSelectedIndex(protUnit == null ? cv.GetParam("Ratio", "PRIDE").Name : protUnit.Name,
				values);
